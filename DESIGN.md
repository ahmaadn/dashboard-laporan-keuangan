...
colors:
  ...
  ash: "#c4b8aa"
  hairline: "#ededed"
  hairline-soft: "#3d2a1a"
  canvas: "#1c1108"
  canvas-soft: "#2c1d10"
  canvas-light: "#ffffff"
  canvas-paper: "#ededed"
  on-canvas-light: "#1c1108"
  link-blue: "#0052ef"
...
**Key Characteristics:**
- Dark-first marketing surface (`{colors.canvas}` `#1c1108`, deep espresso cocoa) with white type and a single coral-red accent (`{colors.brand}`)
...
- **Cocoa-Black** (`{colors.primary}` — `#1c1108`): Default canvas in dark sections; primary action fill on light sections; default text colour on light surfaces. The most-used colour in the system after `{colors.on-primary}`. A deep espresso brown — near-black but carrying the warmth of roasted cocoa, the foundational warmth of the chocolate palette.

### Surface
- **Canvas** (`{colors.canvas}` — `#1c1108`): Default dark-section background. Deep espresso cocoa — near-black with a roasted warmth rather than a neutral graphite.
- **Canvas Soft** (`{colors.canvas-soft}` — `#2c1d10`): Card and panel surface inside dark sections. Used for feature-card-dark and Studio-window mockups. A single-step elevation in cocoa — warm dark brown that lifts cards off the espresso canvas.
...
- **Hairline Soft** (`{colors.hairline-soft}` — `#3d2a1a`): 1-pixel border colour on dark cards and dialog panels.
...
### Text
- **Ink** (`{colors.ink}` — `#1c1108`): Headlines and body on light surfaces. The cocoa-black anchor of the palette.
- **Ink Soft** (`{colors.ink-soft}` — `#2c1d10`): Secondary text on light surfaces; container background colour for dark cards.
- **Graphite** (`{colors.graphite}` — `#3d2a1a`): Tertiary text and dividers in dark sections. The mid-cocoa tone between canvas and canvas-soft.
...
- **Mute** (`{colors.mute}` — `#8a7a6a`): Warm taupe-grey mono caption colour, footer eyebrows. A desaturated cocoa that reads as neutral against the brown canvas.
- **Ash** (`{colors.ash}` — `#c4b8aa`): Default body text on dark surfaces. Warm bone-grey that stays legible against the cocoa canvas without going cold.
