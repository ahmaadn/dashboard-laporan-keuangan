            ...
            <x-app-card eyebrow="Kanal" title="Online vs Offline">
                <div class="row g-2">
                    <div class="col-6">
                        ...
                        <div class="ld-mono-caps">{{ $report['incomeByChannel']['online']['count'] }} trx · {{ $report['incomeByChannel']['online']['qty'] }} unit</div>
                    </div>
                    <div class="col-6">
                        <div class="ld-body-sm text-muted">Offline</div>
                        <div class="tnum fw-medium">@rupiah($report['incomeByChannel']['offline']['total'])</div>
                        <div class="ld-mono-caps">{{ $report['incomeByChannel']['offline']['count'] }} trx · {{ $report['incomeByChannel']['offline']['qty'] }} unit</div>
                    </div>
                </div>
            </x-app-card>

            <x-app-card eyebrow="Admin" title="Penyesuaian HPP">
                <form method="POST" action="/reports/hpp-adjustments" class="ld-form-grid mb-3" onsubmit="return submitHpp(event)">
                    @csrf
                    <div>
                        <label class="form-label">Tanggal</label>
                        <input type="date" name="tanggal" class="form-control form-control-sm" value="{{ today()->toDateString() }}" required>
                    </div>
                    <div>
                            ...
                            <tbody>
                                @foreach ($report['hppPenyesuaian'] as $adj)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <form method="POST" action="/reports/hpp-adjustments/{{ $adj['id'] }}" onsubmit="return deleteHpp(event, this)">
                                                @csrf
                                                @method('DELETE')
