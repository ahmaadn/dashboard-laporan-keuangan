...
@section('content')
    <div x-data="reports()">
            ...
            <x-offcanvas-detail id="offArusKas" eyebrow="Rincian" title="Arus Kas Bersih">
                <div class="d-flex flex-column gap-3">
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <span class="tnum fw-medium">@rupiah($report['hutangTerutang'] ?? $report['hutangPiutang'])</span>
                    </div>
                    --}}
                    <div class="d-flex justify-content-between text-danger">
                        <span class="ld-body-sm">− Pembayaran Hutang dari Kas Usaha</span>
                        <span class="tnum fw-medium">@rupiah($report['pembayaranHutangKas'] ?? 0)</span>
                    </div>
                    <div class="d-flex justify-content-between text-danger">
                    ...
                    <p class="ld-caption mb-0">
                        Arus kas menghitung uang riil keluar-masuk (termasuk modal). Bukan laba. Retur tetap dihitung sebagai
                        kas keluar karena uang dikembalikan ke pelanggan.
                    </p>
                </div>
            </x-offcanvas-detail>
        @endif
        ...
        <x-page-header eyebrow="Reporting" title="Laporan Keuangan">
            <x-slot:actions>
                <x-button variant="secondary" size="sm" icon="download" :disabled="!$report" @click="doExport('PDF')">Ekspor
                    PDF</x-button>
                <x-button variant="secondary" size="sm" icon="download" :disabled="!$report"
                    @click="doExport('Excel')">Ekspor Excel</x-button>
            </x-slot:actions>
        </x-page-header>
        ...
        <div class="ld-filter-bar">
            ...
            <div class="ld-segmented">
                @foreach ($periodOptions as $value => $label)
                    <a href="?period={{ $value }}" class="{{ $activePeriod === $value ? 'is-active' : '' }}">{{ $label }}</a>
                @endforeach
            </div>
            ...
            <x-app-card class="mb-4">
                <div class="ld-validation-notice" role="alert">
                    ...
                    <div>
                        ...
                        <p class="ld-caption mb-0">
                            Perbaiki rentang tanggal terlebih dahulu. Maksimal {{ $maxRentangHari }} hari dan tidak boleh
                            melebihi hari ini.
                        </p>
                    </div>
                </div>
                                ...
                                <div class="col-6">
                                    ...
                                    @if ($channel['retur'] > 0)
                                        <div class="ld-caption text-danger">
                                            Penjualan @rupiah($channel['total']) − Retur @rupiah($channel['retur'])
                                        </div>
                                    @endif
                                </div>
                            @endforeach
                ...
                <x-app-card class="mb-4" eyebrow="Kas" title="Jurnal Arus Kas Bersih">
                    ...
                    @else
                        ...
                        <p class="ld-caption mt-2 mb-0">
                            Saldo berjalan dihitung dari saldo kumulatif sebelum periode. Kas masuk = penjualan + modal riil;
                            kas keluar = pengeluaran + retur + pembayaran hutang dari kas usaha. Pengakuan hutang dan pembayaran
                            dari dana pribadi tidak mengubah kas. Ini arus kas riil, bukan laba.
                        </p>
                    @endif
                </x-app-card>
            @endif
