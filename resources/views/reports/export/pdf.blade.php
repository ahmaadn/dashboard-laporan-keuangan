...
<head>
    ...
    <style>
        ...
        .ld-print__summary td {
            border: 1px solid #e5e7eb;
            padding: 10px 12px;
            width: 25%;
        }

        .ld-print__summary .label {
            font-size: 10px;
        ...
        .ld-print__summary .value {
            font-size: 15px;
            font-weight: 600;
            margin-top: 4px;
        }

        .ld-print__summary .profit {
            color: #047857;
        }

        .ld-print__summary .loss {
            color: #b91c1c;
        }
...
<body>
    ...
    <table class="ld-print__summary">
        <tr>
            ...
            <td>
                ...
                <div class="value {{ $report['labaKotor'] >= 0 ? 'profit' : 'loss' }}">
                    {{ \App\Support\Format::rupiah($report['labaKotor']) }}</div>
            </td>
            <td>
                <div class="label">Laba Bersih</div>
                <div class="value {{ $report['labaBersih'] >= 0 ? 'profit' : 'loss' }}">
                    {{ \App\Support\Format::rupiah($report['labaBersih']) }}</div>
            </td>
        </tr>
    </table>
        ...
        <table class="data">
            <tbody>
                ...
                <tr>
                    ...
                    <td class="num fw-medium {{ $report['labaBersih'] >= 0 ? 'profit' : 'loss' }}">
                        {{ \App\Support\Format::rupiah($report['labaBersih']) }}</td>
                </tr>
            </tbody>
        </table>
    </section>
            ...
            <tbody>
                @foreach (['online' => 'Online', 'offline' => 'Offline'] as $channelKey => $channelLabel)
                    ...
                    <tr>
                        ...
                        <td class="num loss">
                            {{ $channel['retur'] > 0 ? \App\Support\Format::rupiah($channel['retur']) : '—' }}</td>
            ...
            <table class="data">
                ...
                <tbody>
                    @foreach ($report['expenseByCategory'] as $row)
                        <tr>
                            <td class="fw-medium">{{ $row['nama'] }}{{ !empty($row['is_bahan_baku']) ? ' (Bahan Baku)' : '' }}
                            </td>
    ...
    @if (count($journal['entries']) > 0)
        <section>
                ...
                <tbody>
                    ...
                    @foreach ($journal['entries'] as $entry)
                        <tr>
                            ...
                            <td class="num profit">
                                {{ $entry['masuk'] > 0 ? \App\Support\Format::rupiah($entry['masuk']) : '—' }}</td>
                            <td class="num loss">
                                {{ $entry['keluar'] > 0 ? \App\Support\Format::rupiah($entry['keluar']) : '—' }}</td>
            ...
            <p class="ld-print__note">Kas masuk = penjualan + modal; kas keluar = pengeluaran + retur. Arus kas riil, bukan
                laba.</p>
        </section>
    @endif

    <div class="ld-print__foot">
        Dicetak {{ now()->format('d/m/Y H:i') }} · Bahan Baku tercermin via HPP saat terjual · Biaya operasional =
