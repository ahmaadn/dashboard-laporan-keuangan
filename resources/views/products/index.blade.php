...
<div x-data="products(@js($produk), @js($kategoriProduk), @js($currentUser['peran'] === 'admin'))">
                ...
                <tbody>
                    <template x-for="row in visibleRows" :key="row.id">
                        <tr :class="row.dihapus_pada ? 'ld-row-deleted' : ''">
                            ...
                            <td class="text-end">
                                <span class="tnum" :class="row.stok_rendah ? 'text-danger fw-medium' : ''" x-text="row.stok ?? 0"></span>
                                <span class="badge-error-soft ms-1" x-show="row.stok_rendah && !row.dihapus_pada" x-cloak>Rendah</span>
                            </td>
                            <td>
                            ...
                            <td class="text-end" x-cloak>
                                <button type="button" class="ld-action-link" x-show="!row.dihapus_pada" @click="openMovements(row)">Riwayat</button>
                                <template x-if="isAdmin && !row.dihapus_pada">
                                    <span>
                                        <button type="button" class="ld-action-link" @click="openStock(row, 'restok')">+ Stok</button>
                                        <button type="button" class="ld-action-link" @click="openStock(row, 'koreksi')">Sesuaikan</button>
                                        <button type="button" class="ld-action-link" @click="openEdit(row)">Ubah</button>
                                        <button type="button" class="ld-action-link ld-action-link--danger" @click="confirmDelete(row)">Hapus</button>
                                    </span>
                                </template>
            ...
            <div class="ld-modal__body">
                <div class="ld-form-grid">
                    ...
                    <div>
                        ...
                        <input type="number" min="0" step="1000" class="form-control" :class="errors.harga_modal ? 'ld-input-invalid' : ''" x-model="form.harga_modal">
                    ...
                    <div>
                        ...
                        <input type="number" min="0" step="1000" class="form-control" :class="errors.harga_grosir ? 'ld-input-invalid' : ''" x-model="form.harga_grosir" placeholder="Opsional">
    ...
    {{-- Stock modal --}}
    <div class="ld-modal" x-show="stockModal" x-cloak @keydown.escape.window="stockModal = null" @click.self="stockModal = null" x-transition.opacity>
        <div class="ld-modal__dialog" x-transition>
            <div class="ld-modal__header">
                <h5 class="ld-modal__title" x-text="stockForm.aksi === 'restok' ? 'Tambah Stok' : 'Sesuaikan Stok'"></h5>
    ...
    {{-- Movements offcanvas --}}
    <div class="ld-modal" x-show="movementsOpen" x-cloak @keydown.escape.window="movementsOpen = false" @click.self="movementsOpen = false" x-transition.opacity>
