...
@section('content')
    <div
        x-data="dashboard(@js($produk), @js($kategoriProduk), @js($kategoriPengeluaran), @js($pengguna), @js($tanggalHariIni), @js($maxRentangHari), @js($tanggalMulaiUsaha))">

        {{-- 8.7 Filter Periode --}}
        <div class="ld-filter-bar">
            <span class="ld-eyebrow d-none d-sm-inline">Periode</span>
            <div class="ld-segmented">
                <button type="button" :class="period === 'hari_ini' ? 'is-active' : ''" @click="period = 'hari_ini'">Hari
                    Ini</button>
                <button type="button" :class="period === 'minggu_ini' ? 'is-active' : ''"
                    @click="period = 'minggu_ini'">Minggu Ini</button>
                <button type="button" :class="period === 'bulan_ini' ? 'is-active' : ''" @click="period = 'bulan_ini'">Bulan
                    Ini</button>
                <button type="button" :class="period === 'tahun_ini' ? 'is-active' : ''" @click="period = 'tahun_ini'">Tahun
                    Ini</button>
                <button type="button" :class="period === 'rentang' ? 'is-active' : ''"
                    @click="period = 'rentang'">Rentang</button>
            </div>
            <template x-if="period === 'rentang'">
                <div class="d-flex align-items-center gap-2 ms-2">
                    <input type="date" class="form-control form-control-sm" style="max-width: 150px" :min="minDate"
            ...
            <div class="ld-validation-notice mb-3" role="alert">
                ...
                <div>
                    ...
                    <p class="ld-caption mb-0">
                        Data tidak ditampilkan sampai rentang diperbaiki. Maksimal <span x-text="maxRangeDays"></span> hari
                        dan tidak boleh melebihi hari ini.
                    </p>
                </div>
            </div>
        </template>
        ...
        <div class="row g-3 mb-3 align-items-stretch">
            <div class="col-md-6 col-xl-3 d-flex">
                <button type="button" class="stat-card stat-card--income w-100" @click="openCashflow('offPendapatanBersih')"
                    ...
                    <span class="stat-card__value tnum"
                        x-text="fmt(summary.pendapatanBersih ?? summary.income ?? 0)"></span>
                    <span class="stat-card__hint">Penjualan − retur · klik rincian</span>
                </button>
            </div>
            <div class="col-md-6 col-xl-3 d-flex">
                <button type="button" class="stat-card w-100"
                    :class="'stat-card--' + ((summary.labaKotor ?? 0) >= 0 ? 'profit' : 'loss')"
                    @click="openCashflow('offProfit')" title="Pendapatan Bersih − HPP">
            ...
            <div class="col-md-6 col-xl-3 d-flex">
                <button type="button" class="stat-card w-100"
                    :class="'stat-card--' + ((summary.labaBersih ?? summary.profit) >= 0 ? 'profit' : 'loss')"
                    @click="openCashflow('offProfit')" title="Laba Kotor − Beban Operasional">
                    ...
                    <div class="d-flex flex-column gap-3">
                        <div>
                            <div class="d-flex justify-content-between align-items-baseline">
                                ...
                                <span class="tnum fw-medium"
                                    x-text="fmt((incomeByChannel.online && incomeByChannel.online.net_total) || (incomeByChannel.online && incomeByChannel.online.total) || 0)"></span>
                            </div>
                            <div class="ld-channel-bar" role="img" :aria-label="'Online '+ (channelPercent('online')) +'%'">
                                <div class="ld-channel-bar__fill" :style="'width: ' + (channelPercent('online')) + '%'">
                                </div>
                            </div>
                            <div class="ld-mono-caps"
                                x-text="(((incomeByChannel.online && incomeByChannel.online.count) || 0) + ' trx · ' + ((incomeByChannel.online && incomeByChannel.online.qty) || 0) + ' unit · ' + (channelPercent('online')) + '%')">
                            </div>
                        </div>
                        <div>
                            <div class="d-flex justify-content-between align-items-baseline">
                                ...
                                <span class="tnum fw-medium"
                                    x-text="fmt((incomeByChannel.offline && incomeByChannel.offline.net_total) || (incomeByChannel.offline && incomeByChannel.offline.total) || 0)"></span>
                            </div>
                            <div class="ld-channel-bar" role="img"
                                :aria-label="'Offline '+ (channelPercent('offline')) +'%'">
                                <div class="ld-channel-bar__fill ld-channel-bar__fill--offline"
                                    :style="'width: ' + (channelPercent('offline')) + '%'"></div>
                            </div>
                            <div class="ld-mono-caps"
                                x-text="(((incomeByChannel.offline && incomeByChannel.offline.count) || 0) + ' trx · ' + ((incomeByChannel.offline && incomeByChannel.offline.qty) || 0) + ' unit · ' + (channelPercent('offline')) + '%')">
                            </div>
                        </div>
                    </div>
                </x-app-card>
                    ...
                    <div class="d-flex flex-column gap-1">
                        <template x-for="p in (lowStock || [])" :key="p.id">
                            <div class="d-flex justify-content-between">
                                ...
                                <span class="ld-mono-caps tnum text-danger"
                                    x-text="p.stok + ' / min ' + p.stok_minimum"></span>
                            </div>
                        </template>
                    </div>
                </x-app-card>
                            ...
                            <button type="button" class="ld-rank-item" @click="showProductDetail(p.id)">
                                ...
                                <span class="flex-grow-1 text-start">
                                    ...
                                    <span class="ld-mono-caps"
                                        x-text="p.qty + ' unit terjual' + (p.retur_qty ? ' · ' + p.retur_qty + ' diretur' : '') + (p.stok_rendah ? ' · stok rendah' : '')"></span>
                                </span>
                                <span class="ld-mono-caps tnum text-danger ms-2" x-show="p.stok_rendah" x-cloak
                                    title="Stok di bawah minimum">!</span>
                    ...
                    <div class="col-6">
                        ...
                        <template x-if="cmpA === 'rentang'">
                            <div class="d-flex gap-1 mt-1">
                                <input type="date" class="form-control form-control-sm" :min="minDate" :max="today"
                                    x-model="cmpCustomA.start">
                                <input type="date" class="form-control form-control-sm" :min="minDate" :max="today"
                                    x-model="cmpCustomA.end">
                                <x-button variant="secondary" size="sm" icon="close" iconOnly @click="clearCompareA()"
                                    title="Bersihkan rentang Periode A" />
                            </div>
                        </template>
                    </div>
                    <div class="col-6">
                        ...
                        <template x-if="cmpB === 'rentang'">
                            <div class="d-flex gap-1 mt-1">
                                <input type="date" class="form-control form-control-sm" :min="minDate" :max="today"
                                    x-model="cmpCustomB.start">
                                <input type="date" class="form-control form-control-sm" :min="minDate" :max="today"
                                    x-model="cmpCustomB.end">
                                <x-button variant="secondary" size="sm" icon="close" iconOnly @click="clearCompareB()"
                                    title="Bersihkan rentang Periode B" />
                            </div>
                        </template>
                    </div>
                </div>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="text-end tnum"
                                    x-text="fmt(cmpSummaryA.pendapatanBersih ?? cmpSummaryA.income ?? 0)"></td>
                                <td class="text-end tnum"
                                    x-text="fmt(cmpSummaryB.pendapatanBersih ?? cmpSummaryB.income ?? 0)"></td>
                                <td class="text-end tnum" :class="cmpDeltaClass('pendapatanBersih')"
                                    x-text="cmpDelta('pendapatanBersih')"></td>
                            </tr>
                            <tr>
                                <td>Pengeluaran</td>
                                <td class="text-end tnum" x-text="fmt(cmpSummaryA.expense)"></td>
                                <td class="text-end tnum" x-text="fmt(cmpSummaryB.expense)"></td>
                                <td class="text-end tnum" :class="cmpDeltaClass('expense')" x-text="cmpDelta('expense')">
                                </td>
                            </tr>
                            <tr>
                                <td>Laba Kotor</td>
                                ...
                                <td class="text-end tnum" :class="cmpDeltaClass('labaKotor')"
                                    x-text="cmpDelta('labaKotor')"></td>
                            </tr>
                            <tr>
                                <td>Laba Bersih</td>
                                <td class="text-end tnum" x-text="fmt(cmpSummaryA.labaBersih ?? cmpSummaryA.profit ?? 0)">
                                </td>
                                <td class="text-end tnum" x-text="fmt(cmpSummaryB.labaBersih ?? cmpSummaryB.profit ?? 0)">
                                </td>
                                <td class="text-end tnum" :class="cmpDeltaClass('labaBersih')"
                                    x-text="cmpDelta('labaBersih')"></td>
                            </tr>
                            <tr>
                                ...
                                <td class="text-end tnum" :class="cmpDeltaClass('arusKasBersih')"
                                    x-text="cmpDelta('arusKasBersih')"></td>
                            </tr>
                        </tbody>
                    </table>
                </x-data-table>
                        ...
                        <template x-for="r in recentTransactions" :key="r.type + '-' + r.id">
                            <button type="button" class="ld-recent-item" @click="showTransaction(r)">
                                <span class="ld-recent-item__type">
                                    <span class="badge"
                                        :class="r.type === 'pemasukan' ? 'badge-success-soft' : 'badge-error-soft'"
                                        x-text="r.type === 'pemasukan' ? 'Masuk' : 'Keluar'"></span>
                                    <span class="ms-1"
                                ...
                                <span class="flex-grow-1 text-start">
                                    ...
                                    <span class="ld-mono-caps"
                                        x-text="new Date(r.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' }) + ' · ' + (penggunaMap[r.id_pengguna]?.nama || '—')"></span>
                                </span>
            ...
            <div class="d-flex flex-column gap-3">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <span class="tnum fw-bold"
                        :class="(summary.labaBersih ?? summary.profit) >= 0 ? 'text-success' : 'text-danger'"
                        x-text="fmt(summary.labaBersih ?? summary.profit ?? 0)"></span>
                </div>
                <hr class="my-1">
                <div class="d-flex justify-content-between">
        ...
        <x-offcanvas-detail id="offArusKas" eyebrow="Rincian" title="Arus Kas Bersih">
            <div class="d-flex flex-column gap-3">
                ...
                {{--
                <div class="d-flex justify-content-between">
                    <span class="ld-body-sm">Hutang Berjalan (non-kas)</span>
                    <span class="tnum fw-medium" x-text="fmt(summary.hutangTerutang ?? summary.hutangPiutang ?? 0)"></span>
                </div>
                --}}
                <div class="d-flex justify-content-between text-danger">
                    <span class="ld-body-sm">− Pembayaran Hutang dari Kas Usaha</span>
                    <span class="tnum fw-medium" x-text="'− ' + fmt(summary.pembayaranHutangKas ?? 0)"></span>
                </div>
                <div class="d-flex justify-content-between text-danger">
                ...
                <div class="d-flex justify-content-between fw-bold">
                    ...
                    <span class="tnum" :class="(summary.arusKasBersih ?? 0) >= 0 ? 'text-success' : 'text-danger'"
                        x-text="fmt(summary.arusKasBersih ?? 0)"></span>
                </div>
                <p class="ld-caption mb-0">
                    Arus kas menghitung uang riil keluar-masuk, termasuk modal riil dan pembayaran hutang dari kas usaha.
                    Bukan laba.
                    Pengakuan hutang dan pembayaran dari dana pribadi tidak mengubah kas. Retur tetap dihitung sebagai kas
                    keluar karena uang dikembalikan ke pelanggan.
                </p>
            </div>
        </x-offcanvas-detail>
