...
<div x-data="salesReturns(@js($retur), @js($currentUser))">
    ...
    <x-page-header eyebrow="Transaksi" title="Retur Penjualan">
        <x-slot:actions>
            <button type="button" class="btn btn-brand" @click="openAdd()">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="12" y1="5" x2="12" y1="19"/><line x1="5" y1="12" x2="19" y1="12"/></svg>
                Catat Retur
            </button>
        </x-slot:actions>
    </x-page-header>
    ...
    <div class="alert alert-info mb-4" role="status">
        Retur penjualan otomatis dicatat sebagai <strong>pengurang pendapatan</strong> (Pendapatan Bersih = Penjualan − Retur).
        Stok barang yang diretur akan dikembalikan.
    </div>

    <x-app-card class="mb-4">
        <div class="d-flex justify-content-between align-items-center mb-3 gap-2">
            <input type="search" class="form-control" style="max-width: 320px" placeholder="Cari tanggal, penjualan, alasan…" x-model="search">
                ...
                <tbody>
                    <template x-for="row in visibleRows" :key="row.id">
                        <tr :class="row.dihapus_pada ? 'ld-row-deleted' : ''">
                            ...
                            <td x-text="row.id_produk ? ('Produk #' + row.id_produk) : '—'"></td>
                            ...
                            <td class="text-end">
                                <button type="button" class="ld-action-link ld-action-link--danger" x-show="!row.dihapus_pada && isAdmin" @click="confirmDelete(row)">Hapus</button>
            ...
            <div class="ld-modal__body">
                <div class="ld-form-grid">
                    <div class="full">
                        ...
                        <input type="number" min="1" class="form-control" :class="errors.id_penjualan ? 'ld-input-invalid' : ''" x-model="form.id_penjualan" placeholder="Masukkan ID penjualan (mis. 12)">
                    ...
                    <div class="full">
                        ...
                        <textarea class="form-control" rows="2" x-model="form.alasan" placeholder="mis. Barang cacat, Ukuran tidak sesuai"></textarea>
                    </div>
                </div>
    ...
    <div class="ld-modal" x-show="deleteTarget" x-cloak @keydown.escape.window="deleteTarget = null" @click.self="deleteTarget = null" x-transition.opacity>
        <div class="ld-modal__dialog" style="max-width: 420px" x-transition>
            <div class="ld-modal__header"><h5 class="ld-modal__title">Hapus Retur?</h5></div>
            <div class="ld-modal__body">
                <p class="mb-0">Retur penjualan <strong>#<span x-text="deleteTarget?.id_penjualan"></span></strong> sebesar <strong x-text="deleteTarget ? rupiah(deleteTarget.nominal_retur) : ''"></strong> akan dihapus. Stok TIDAK otomatis dikembalikan — untuk membatalkan penjualan, hapus transaksi asal di halaman Pemasukan.</p>
            </div>
            <div class="ld-modal__footer">
